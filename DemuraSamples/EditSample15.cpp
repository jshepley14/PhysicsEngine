...
const dReal sphrCenter[3] = {S1x=0.0,S1y=0,S1z=1.9}; // length of edges
...
//Feed in the function only the radius, the only the center, the only the MyObject and see where the code breaks.
//don't forget to alter the input in the other places the function is called
...
void restart()
{
    ...
    //createBall();
    createBall(ball, sphrCenter, radius );
    createPillar();
}
